...
if [[ "$os" == "linux-gnu"* ]]; then
    if [[ -f ${sign_local_path} ]]; then
        echoca blue "Gecko tarball signature already exists at ${sign_local_path}"
    else
        echoca blue "Downloading Gecko tarball signature from ${sign_url}"
        wget -P ${gecko_dir} ${sign_url}
    fi

    echoca blue "Verifying signature from ${sign_local_path}"
